...
var matches = function matches(filepath, key, list_or_string) {
    ...
    if (list_or_string) {
        matched = typeof list_or_string == "string" ? ignore.sync(list_or_string) : ignore.compile(list_or_string.join("\n"));
    } else {
...
var is_allowed = function is_allowed(filepath, allow_config) {
    return _.isEmpty(allow_config) ? true : matches(filepath, "vile.allow", allow_config);
};
// TODO: make io async
...
var spawn = function spawn(bin) {
    ...
    return new Bluebird(function (resolve, reject) {
        ...
        var proc = execa.spawn(bin, opts.args, {

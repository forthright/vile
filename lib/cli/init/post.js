...
var install_plugin_args = function install_plugin_args(plugins) {
    return _.concat("install", "--save-dev", "@forthright/vile",
    // TODO: when public, shoule not have @forthright prefix
    _.reduce(plugins, function (cmd, plugin) {
...
var install_peer_and_alt_lang_dependencies = function install_peer_and_alt_lang_dependencies(config) {
    // TODO: move to method
    ...
    return inquirer.prompt([{
        type: "confirm",
        name: "ok_to_continue",
        message: "Install required plugin peer dependencies? " + ("(requires " + Object.keys(by_bin).join(",") + ")"),
        default: true
    }]).then(function (answers) {
...
var ready_to_punish = function ready_to_punish(config) {
    ...
    log.info("    ~$ VILE_API_TOKEN=XXXXXXX vile p " + "-u project_name");
    log.info();
...
module.exports = {
    init: function init(config) {
        return confirm_vile_config_is_ok(config).then(create_config).then(install_peer_and_alt_lang_dependencies).then(install_plugins).then(ready_to_punish);
    }
};
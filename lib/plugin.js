    ...
    return function (plugins) {
        return new Bluebird(function (resolve, reject) {
            ...
            cluster.on("fork", function (worker) {
                ...
                log.info(workers[worker.id] + ":start " + ("(" + worker.id + "/" + plugin_count + ")"));
                if (spin) spin.start();
            });
...
var run_plugins = function run_plugins() {
    ...
    return fs.readdirAsync(cwd_plugins_path()).filter(is_plugin).then(execute_plugins(plugins, config, opts)).then(opts.snippets ? add_code_snippets() : passthrough).then(lookup_ok_issues ? add_ok_issues(allow, ignore, opts.scores) : passthrough);
};
module.exports = {
    exec: run_plugins,

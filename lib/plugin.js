    ...
    return function (plugins) {
        return new Bluebird(function (resolve, reject) {
            ...
            Bluebird.map(plugins, function (plugin) {
                ...
                return run_plugins_in_fork(plugins_to_run, config, worker).then(function (issues) {
                    if (spin) spin.stop(true);
...
var run_plugins = function run_plugins() {
    ...
    return fs.readdirAsync(cwd_plugins_path()).filter(is_plugin).then(execute_plugins(plugins, config, opts)).then(post_process ? log_post_process("start") : passthrough).then(opts.snippets ? add_code_snippets() : passthrough).then(post_process ? add_ok_issues(allow, ignore, opts.scores) : passthrough).then(post_process ? log_post_process("finish") : passthrough);
};
module.exports = {
    exec: run_plugins,

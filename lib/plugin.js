...
var exec_plugin = function exec_plugin(name) {
...
var exec_in_fork = function exec_in_fork(plugins, config, worker) {
    return new Bluebird(function (resolve, reject) {
...
var on_windows = function on_windows() {
    return (/windows/i.test(os.type())
    );
};
...
var combine_paths = function combine_paths(combine_str, issues) {
    var sanitized_combine_paths = _.map(combine_str.split(","), function (def) {
        return def.split(":");
    });
    ...
    return function (plugins) {
        return new Bluebird(function (resolve, reject) {
            ...
            Bluebird.map(plugins, function (plugin) {
                ...
                return exec_in_fork(plugins_to_run, config, worker).then(function (issues) {
                    if (spin) spin.stop(true);
...
var log_post_process = function log_post_process(state) {
    return function (issues) {
...
var exec = function exec() {
    ...
    return fs.readdirAsync(cwd_plugins_path()).filter(is_plugin).then(execute_plugins(plugins, config, opts)).then(post_process ? log_post_process("start") : passthrough).then(post_process && allow_snippets ? add_code_snippets() : passthrough).then(post_process ? add_ok_issues(allow, ignore) : passthrough).then(post_process ? log_post_process("finish") : passthrough);
};
module.exports = {
    exec: exec,

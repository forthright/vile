...
(function (vile) {
    ...
    var add_ok_issues = function add_ok_issues(vile_ignore) {
        return function (issues) {
            ...
            function (p) {
                ...
                return _.reject(ok_issues, function (issue) {
    ...
    var run_plugins = function run_plugins() {
        ...
        return fs.readdirAsync(cwd_plugins_path()).filter(is_plugin).map(into_executed_plugins(plugins, config, opts), { concurrency: 1 }).then(_.flatten).then(add_ok_issues(app_config.ignore))["catch"](error_executing_plugins);
    };
    module.exports = {
        exec: run_plugins,

...
var combine_paths = function combine_paths(combine_str, issues) {
    ...
    _.each(combine_paths, function (paths) {
        ...
        var base_path = _paths[0];
        var merge_path = _paths[1];

        var merge_path_regexp = new RegExp("^" + merge_path + "/", "i");
        _.each(issues, function (issue, idx) {
            ...
            if (merge_path_regexp.test(issue_path)) {
                ...
                var base_data_exists = _.some(issues, function (i) {
                    return i && i.path == issue_path && i.type == issue_type;
                });

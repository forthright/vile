...
(function (vile) {
    ...
    var add_code_snippets = function add_code_snippets() {
        var vile_ignore = arguments.length <= 0 || arguments[0] === undefined ? [] : arguments[0];
        return function (issues) {
            return Bluebird.map(issues, function (issue) {
                var start = Number(_.get(issue, "where.start.line", 0));
                var end = Number(_.get(issue, "where.end.line", start));
                if (_.any(util.displayable_issues, function (t) {
                    return t == issue.type;
                }) && _.has(issue, "path") && fs.statSync(issue.path).isFile()) {
                    return fs.readFileAsync(path.join(process.cwd(), issue.path), "utf-8").then(function (data) {
                        if (start === 0 && end === start) return issue;
                        var lines = linez(data).lines;
                        issue.snippet = lines.reduce(function (snippets, line, num) {
    ...
    var run_plugins = function run_plugins() {
        ...
        return fs.readdirAsync(cwd_plugins_path()).filter(is_plugin).then(execute_plugins(plugins, config, opts)).then(opts.snippets ? add_code_snippets(ignore) : passthrough).then(add_ok_issues(ignore)).catch(error_executing_plugins);
    };
    module.exports = {
        exec: run_plugins,

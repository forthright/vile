...
(function (vile) {
    ...
    var to_console = function to_console(issue) {
        ...
        var loc = h_line || h_char ? "line " + (h_line || "?") + ", col " + (h_char || "?") + ", " : "";
    ...
    var log_plugin_messages = function log_plugin_messages(name) {
        ...
        var nlog = logger.create(name);
        issues.forEach(function (issue, index) {
            if (issue.type == util.ERROR || issue.type == util.WARNING || issue.type == util.INFO) {
                // TODO: lookup/log unknown logs
                var _print = nlog[issue.type || "error"];
                _print(to_console(issue));
            }
        });
    };
    ...
    var failed = function failed(list) {
        return _.select(list, function (item) {
            return item.type == util.ERROR || item.type == util.WARNING;
        }).length > 0;
    };

...
var run_plugins = function run_plugins() {
    ...
    return fs.readdirAsync(cwd_plugins_path()).filter(is_plugin).then(execute_plugins(plugins, config, opts)).then(opts.snippets ? add_code_snippets() : passthrough).then(add_ok_issues(allow, ignore)).catch(error_executing_plugins);
};
module.exports = {
    exec: run_plugins,

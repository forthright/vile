...
var handle_response = function handle_response(resolve, reject) {
    return function (err, response, body) {
        return err ? reject({ error: err }) : resolve({ body: body, response: response });
    };
};
...
var log_summary = function log_summary(post_json) {
    ...
    _.each(files, function (file) {
        log.info(padded_file_score(_.get(file, "score", 0)) + " => " + ("" + _.get(file, "path")));
    });
    log.info();

...
(function (vile_1) {
    ...
    var log_vileio_summary = function log_vileio_summary(log, info, verbose) {
        // HACK
        var score = _.get(info, "score");
        var files = _.get(info, "files");
        var new_issues = _.get(info, "new_issues");
        var review_url = _.get(info, "review_url");
    ...
    var publish = function publish(issues, opts) {
        ...
        return service.commit(issues, config.get_auth()).then(function (http) {
            if (_.get(http, "response.statusCode") == 200) {
                log_vileio_summary(log, _.attempt(JSON.parse.bind(null, http.body)), opts.scores);
            } else {
                log.error(http.body);
            }
        }).catch(function (err) {
    ...
    var punish = function punish(app) {
        ...
        vile.exec(parse_plugins(plugins), app.config, app).then(function (issues) {
